...
pub struct SeccompPolicy {
    /// Built-in preset name. Currently supported: `"nodejs"`, `"custom"`, `"disabled"`.
    #[serde(default)]
    ...
    /// Explicit syscall allowlist (only used when `preset = "custom"`).
    /// Each entry is a syscall name (e.g., `"read"`, `"write"`, `"mmap"`).
    /// Generated by `gleisner learn` from seccomp audit observations.
...
pub enum SeccompPreset {
    ...
    Nodejs,
    /// Explicit syscall allowlist from `allow_syscalls`.
    /// Generated by `gleisner learn` from seccomp audit observations.
    Custom,
}
