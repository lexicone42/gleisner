...
- `lake build` runs the Lean 4 C++ kernel for packages with `verified_properties`
- Dual-kernel verification via `lean4export` + `nanoda_bin` (independent Rust type checker)
- Verification results embedded in attestation as `VerifiedProperty` structs with `verified_by_forge` and `dual_verified` fields
- `--strict-verify` makes any failure a hard stop
- Graceful degradation: missing Lean binary skips verification, nanoda failure falls back to single-kernel

**Known limitation:** nanoda v0.4.3-beta panics on `_native.bv_decide` axioms generated by modern Lean 4's `bv_decide` tactic. A fork is planned to handle these axioms.
