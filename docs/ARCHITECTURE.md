...
Seccomp-BPF restricts which system calls the sandboxed process can invoke. The filter is installed after Landlock and before exec, using `seccompiler` (the pure-Rust BPF compiler from Firecracker):

| Preset | Description |
|--------|-------------|
| `disabled` | No filtering (default) |
| `nodejs` | Allowlist tuned for Node.js/V8 — permits JIT (`mmap`, `mprotect`), networking, filesystem, and process management while blocking dangerous syscalls (`mount`, `ptrace`, `bpf`, `io_uring_*`, `init_module`, `kexec_load`) |
| `custom` | Explicit allowlist generated by `gleisner learn` from real session observations |

The `default_action` controls what happens when a blocked syscall is attempted:

| Action | Effect | Use Case |
|--------|--------|----------|
| `errno` | Returns EPERM, process continues | Default — debuggable, non-destructive |
| `log` | Allows but logs via kernel audit (type 1326) | Learning mode — discover which syscalls are actually needed |
| `kill` | Kills the process immediately | Maximum security for production |

The learning workflow: run a session with `default_action = "log"`, then `gleisner learn --kernel-audit-log /var/log/audit/audit.log` parses the SECCOMP audit records to build a `custom` allowlist tailored to the exact syscalls observed.
...
Profiles are TOML files defining all five isolation dimensions:
...
The five bundled profiles (named after polises in *Diaspora*):
...
- **developer** -- Development-focused. Full Rust toolchain access (cargo, rustup, crates.io, GitHub), generous resource limits (16GB memory), MCP server package registries, seccomp `nodejs` preset. Designed for gleisner-in-gleisner self-hosting.
- **developer-open** -- Unrestricted development. Network allow-all, no seccomp, generous limits. For trusted environments where the sandbox provides isolation without policy constraints.
...
pub enum EventKind {
    ...
    Syscall { number, name },        // from seccomp audit log (learning mode)
}
```
